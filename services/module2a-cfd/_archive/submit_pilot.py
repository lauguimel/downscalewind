...
GROUPS = {
    ...
    "phase0_resolution": {
        ...
        "walltime": "6:00:00",   # 100m/9.3M cells: ~4h on 24 cores with SIMPLEC
        "mem_gb": 32,
        "ncpus": 24,
    },
    # ── Phase 1 — Cylindrical domain, 3 rings, R=14km (BBSF neutral) ──
    "phase1_cylinder": {
        "manifest": None,   # auto-generated from configs/phase1_cylinder.yaml
        "local_dir": PROJECT_ROOT / "data" / "cases" / "phase1_cylinder",
        "walltime": "6:00:00",   # 2-5M cells: ~2h on 24 cores
        "mem_gb": 32,
        "ncpus": 24,
    },
}
...
def _load_yaml_manifest(cfg_name: str) -> dict:
    """Generic manifest loader from a study YAML (phase0_resolution, phase1_cylinder...)."""
    import yaml
    ...
    for case_id, case_cfg in cfg["cases"].items():
        manifest[f"case_{case_id}"] = {
            ...
            "domain_type": case_cfg.get("domain_type", cfg["study"].get("domain_type", "box")),
            "domain_km": case_cfg.get("domain_km", cfg["study"].get("domain_km", 10)),
...
# ── PBS template ────────────────────────────────────────────────────────
# Rendered at submit time (remote_dir and ncpus are only known then).
# Uses the HPC module system directly — no container wrapper needed since
# cartesianMesh is bundled in OpenFOAM/v2406 on Aqua.
PBS_TEMPLATE = """\
#!/bin/bash -l
#PBS -N {job_name}
...
# Clean previous run artifacts so rsync-uploaded files are not mixed
# with stale HPC-generated output (processor dirs, logs, postProcessing)
rm -rf log.* processor* postProcessing

module load GCCcore/12.3.0 GCC/12.3.0 OpenMPI/4.1.5 OpenFOAM/v2406
source $EBROOTOPENFOAM/OpenFOAM-v2406/etc/bashrc

# Ensure OF bin (cartesianMesh, solvers) is visible to child processes
export PATH="$FOAM_APPBIN:$WM_PROJECT_DIR/bin:$PATH"
echo "cartesianMesh: $(which cartesianMesh 2>&1)"

# Activate Python venv for field reconstruction (numpy + zarr)
# Created once with: qsub configs/hpc/setup_python_env.pbs
if [ -f "$HOME/venv/reconstruct/bin/activate" ]; then
    source "$HOME/venv/reconstruct/bin/activate"
    echo "Python venv activated: $(python3 --version)"
else
    echo "WARNING: $HOME/venv/reconstruct not found — Python reconstruction may fail"
fi
...
def _get_manifest(group_name: str, group_cfg: dict) -> dict:
    ...
    if group_name == "phase1_cylinder":
        return _load_yaml_manifest("phase1_cylinder.yaml")
...
def submit_cases(dry_run: bool = False, group_filter: str | None = None) -> dict[str, str]:
    ...
    for group_name, group_cfg in GROUPS.items():
        ...
        for case in cases:
            ...
            # Render PBS script with correct remote_dir and ncpus, then upload.
