...
List<gradingDescriptors> BlockConventions::gradingFactors2ExpandRatios(
			const scalarList & gradingFactors
			){
	List<gradingDescriptors> out(12);

    // x-direction (edges 0-3)
    for (label i = 0; i < 4; i++)
        out[i] = gradingDescriptors(gradingDescriptor(1, 1, gradingFactors[0]));

    // y-direction (edges 4-7)
    for (label i = 4; i < 8; i++)
        out[i] = gradingDescriptors(gradingDescriptor(1, 1, gradingFactors[1]));

    // z-direction (edges 8-11)
    for (label i = 8; i < 12; i++)
        out[i] = gradingDescriptors(gradingDescriptor(1, 1, gradingFactors[2]));
